 ...
 * {@link Weather}, ...). It exists so the network manager's {@code send(Entity, String)} can take
 * one parameter type instead of one overload per object type. Delete this file and use your actual
 * import instead.
 * <p>
 * Whatever fields the real {@code Entity} declares are <b>the network layer's to populate at send
 * time</b> (see {@code planesim.core.network.NetworkWriter}), never {@code planesim.core}'s — the simulation only ever
